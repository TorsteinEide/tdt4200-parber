...
swap ( real_t** m1, real_t** m2 )
{
    ...
    // Create a temporary variable to store the value pointed to by m1
    real_t* temp = *m1;
    // Store the value pointed to by m2 in m1
    *m1 = *m2;
    // Store the m1 value temporarily stored in the temp variable into m2
    *m2 = temp;
}
...
main ( int argc, char** argv )
{
    ...
    // check if null, if not parsin was successfull
    if (options) {
        // Store values from options into corresponding global variables
        N = options->N;
        max_iteration = options->max_iteration;
        snapshot_frequency = options->snapshot_frequency;
    } else { // If options is null exit with code 1 (failure)
        exit(1);
    }
...
domain_init ( void )
{
    ...
    temp[0] = (real_t*)malloc(N * sizeof(real_t));
    // Create the second pointer
    temp[1] = (real_t*)malloc(N * sizeof(real_t));
    // Third pointer
    temp[2] = (real_t*)malloc(N * sizeof(real_t));
